"""
visualize_results.py  —  V5 Edition
Generates publication-quality output images and spectral plots for all 5 models.

Saves to version5/visualization/outputs/:
  ...
  - spectral_gradient.png            — band-difference gradient curves
...
def hsi_to_rgb(t, bands=(30, 20, 10)):
    """(C,H,W) tensor/array → (H,W,3) numpy float32 in [0,1]."""
    if hasattr(t, 'cpu'):
        t = t.cpu()
    if hasattr(t, 'numpy'):
        t = t.numpy()
...
def main():
    ...
    hr_hsi = batch['hr_hsi'].unsqueeze(0)
    ...
    models = {v: load_model(v, ck, device) for v, ck in ckpts.items()}
    ...
    for v, pred in preds.items():
        ...
        label = (f"{VARIANT_LABELS[v]}\n"
                 f"PSNR={mi['psnr']:.2f}dB  SAM={mi['sam']:.3f}  ERGAS={mi['ergas']:.3f}")
    ...
    axes[0].imshow(hsi_to_rgb(lr_up[0])); axes[0].set_title('LR-HSI\n(Bicubic)', fontsize=9); axes[0].axis('off')
    axes[1].imshow(hr_pan[0, 0].cpu().numpy(), cmap='gray'); axes[1].set_title('HR-PAN', fontsize=9); axes[1].axis('off')
    axes[2].imshow(hsi_to_rgb(hr_hsi[0])); axes[2].set_title('GT HR-HSI', fontsize=9); axes[2].axis('off')

    for ci, v in enumerate(VARIANTS, 3):
        mi = metrics[v]
        axes[ci].imshow(hsi_to_rgb(preds[v][0].clamp(0, 1)))
        axes[ci].set_title(f"{VARIANT_LABELS[v]}\nPSNR={mi['psnr']:.2f}  SAM={mi['sam']:.3f}", fontsize=7)
        axes[ci].axis('off')
    ...
    if len(VARIANTS) == 1:
        axes2 = [axes2]
    all_err_max = max((preds[v][0] - hr_hsi[0]).abs().mean(0).numpy().max() for v in VARIANTS)
    ...
    for ax, v in zip(axes2, VARIANTS):
        ...
        ax.set_title(f"{VARIANT_LABELS[v]}\nPSNR={metrics[v]['psnr']:.2f}  ERGAS={metrics[v]['ergas']:.3f}", fontsize=8)
        ax.axis('off')
    ...
    for v in VARIANTS:
        ...
        ax.plot(bands, spec, '--', color=COLOURS[v], linewidth=1.5, label=VARIANT_LABELS[v])
    ax.set_xlabel('Band index', fontsize=11); ax.set_ylabel('Reflectance', fontsize=11)
    ax.set_title(f'Spectral Curve at Pixel ({py}, {px})', fontsize=12)
    ax.legend(fontsize=7); ax.grid(True, alpha=0.3)

    # Spectral gradient (band differences)
    ax2 = axes3[1]
    ...
    for v in VARIANTS:
        spec = preds[v][0, :, py, px].numpy()
        grad = np.diff(spec)
        ax2.plot(bands[1:], grad, '--', color=COLOURS[v], linewidth=1.5, label=VARIANT_LABELS[v])
    ax2.axhline(0, color='gray', linewidth=0.8, linestyle=':')
    ax2.set_xlabel('Band index', fontsize=11); ax2.set_ylabel('Band difference', fontsize=11)
    ax2.set_title('Spectral Gradient — shape preservation', fontsize=12)
    ...
    # ── Metric summary ────────────────────────────────────────────────────────
    print("\n" + "=" * 74)
    print(f"{'Variant':<32} {'PSNR (↑)':>10} {'SAM (↓)':>10} "
          f"{'ERGAS (↓)':>10} {'Params':>8}")
    print("=" * 74)
    for v in VARIANTS:
        m = metrics[v]
        print(f"  {VARIANT_LABELS[v]:<30} {m['psnr']:>10.3f} "
              f"{m['sam']:>10.3f} {m['ergas']:>10.3f} "
              f"{m['params']/1e6:>7.3f}M")
    print("=" * 74)
