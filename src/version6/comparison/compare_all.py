"""
compare_all.py  —  V6 Edition
Trains V6 model variant (spectral-focused loss) and saves results.

Usage
─────
  cd version6
  python comparison/compare_all.py --epochs 200 --batch_size 1 --patch_size 32
  python comparison/compare_all.py --epochs 200 --skip_old --skip_improved --skip_v3 --skip_v4 --skip_v5
"""
...
CompositeLossV5     = _lf5.CompositeLossV5
...
def save_images(epoch, variant, lr_hsi, hr_pan, pred, gt, out_dir):
    ...
    lr_up = F.interpolate(lr_hsi.cpu(), scale_factor=SCALE, mode='bicubic', align_corners=False)
...
def save_metric_plots(variant, history, epoch, out_dir):
    ...
    fig.suptitle(f'{VARIANT_LABELS[variant]}  —  Epoch {epoch}', fontsize=14, color='white', y=1.01)
    ...
    for ax, (vals, title, ylabel, better, c) in zip(axes, metrics):
        ...
        if valid_pairs:
            ...
            ax.axvline(best_e, color='#ffcc00', linewidth=1.2, linestyle='--', alpha=0.8, zorder=2)
            ax.scatter([best_e], [best_v], color='#ffcc00', zorder=5, s=50)
            ax.text(best_e+0.3, best_v, f'{best_v:.3f}', fontsize=8, color='#ffcc00', va='center', zorder=6)
    ...
    fig.savefig(os.path.join(out_dir, fname), dpi=130, bbox_inches='tight', facecolor='#0d1117')
    plt.close(fig)
    print(f'    [Metric plot saved -> {os.path.join(out_dir, fname)}]')


def get_loss_for_variant(variant):
    """V6 uses CompositeLossV6; all others use CompositeLossV5."""
    if variant == 'v6':
        return CompositeLossV6()
    return CompositeLossV5()
...
def train_variant(variant, args, device, train_l, val_l):
    ...
    criterion = get_loss_for_variant(variant)
    if variant == 'v6':
        print(f"  Loss: CompositeLossV6  (lambda_sam=0.40, lambda_spec=0.10)")
    else:
        print(f"  Loss: CompositeLossV5  (lambda_sam=0.10, lambda_spec=0.05)")
    ...
    epoch_iter = tqdm(range(1, args.epochs + 1), desc=f"[{variant}] epochs", unit='ep') if HAS_TQDM else range(1, args.epochs + 1)
    for epoch in epoch_iter:
        model.train()
        train_loss = 0.0
        t0 = time.time()

        train_pbar = tqdm(train_l, desc=f"  Train {epoch:3d}/{args.epochs}", leave=False, unit='batch') if HAS_TQDM else train_l
        ...
        if HAS_TQDM:
            tqdm.write(msg)
            epoch_iter.set_postfix(psnr=f'{val_psnr:.2f}', sam=f'{val_sam:.3f}')
        else:
            print(msg)
...
def make_comparison_plot(results):
    ...
    plt.suptitle('V6 Model Comparison', fontsize=12)
    plt.tight_layout()
...
def save_excel(results):
    ...
    ws = wb.active
    ws.title = 'V6 Comparison'
    ws.append(['Variant', 'Label', 'Params', 'Best PSNR', 'Best SAM', 'Best ERGAS', 'Best SSIM', 'Checkpoint'])
    for r in results:
        ws.append([r['variant'], r['label'], r['params'],
                   r['best_psnr'], r['best_sam'], r['best_ergas'],
                   r['best_ssim'], r['checkpoint']])
...
def main():
    ...
    parser.add_argument('--skip_v6',         action='store_true')
    ...
    train_l, val_l, _ = create_dataloaders_overlap(
        ...
        overlap=args.overlap, scale=args.scale, num_workers=0,
    )
    ...
    if not args.skip_v6:       variants_to_run.append('v6')
    ...
    print("\n" + "=" * 85)
    print(f"{'Variant':<35} {'PSNR (+)':>10} {'SAM (-)':>10} {'ERGAS (-)':>10} {'Params (M)':>12}")
    print("=" * 85)
    for r in results:
        print(f"  {r['label']:<33} {r['best_psnr']:>10.3f} "
